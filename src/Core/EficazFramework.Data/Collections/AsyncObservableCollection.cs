...
namespace EficazFramework.Collections
{
    public class AsyncObservableCollection<T> : ObservableCollection<T>
    {
        ...
        {
            if (ReferenceEquals(SynchronizationContext.Current, Utilities.Threading.Thread.SynchronizationContext) || Utilities.Threading.Thread.SynchronizationContext  == null)
            {
                action();
            }
            else
            {
                Utilities.Threading.Thread.SynchronizationContext.Send(new SendOrPostCallback((object state) => action()), null);
            }
        }
